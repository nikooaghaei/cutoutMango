...
def find_main_part(loader):######newwwwwwwwww
    ...
    #        for image in images:
            for i in range (128):
                ...
                mango = Mango(cnn, images[i])
#                print("true label:" + str(labels[i]))

                main_nodes.append(mango())

                ###printing results
                mango.show_chain(str(i))
...
for epoch in range(args.epochs):    ##no testing
...
#    test_acc = test(test_loader)  ##no testing

#    tqdm.write('test_acc: %.3f' % (test_acc))  ##no testing

    scheduler.step(epoch)  # Use this line for PyTorch <1.4
    # scheduler.step()     # Use this line for PyTorch >=1.4

#    row = {'epoch': str(epoch), 'train_acc': str(accuracy), 'test_acc': str(test_acc)} ##no testing
    row = {'epoch': str(epoch), 'train_acc': str(accuracy)}

main_parts = find_main_part(test_loader)    ##Newwwwwwwww
for i in range(len(main_parts)):
    save_image(main_parts[i].data, str(i) + '/result.png')
#torch.set_printoptions(profile="full")
#print(main_parts[0].permute(1,2,0))
