...
class Mango(object):
    ...
    """
    def __init__(self, model, root_img, n_masks = 4):
        self.model = model
        self.root = Tree(root_img)
        self.n_masks = n_masks
        #self.treshold = 1 ##########################TO BE SET??????
        self.root_label = None
        self.res = None #variable to keep main part of self obj image (by calling make_tree func)

        self.test = 0
    def __call__(self):    ######not sure if we should have __call?
        """
        Args:
...
#        if(self.root_label != self.temp_label):
 #           print("true label: " + str(self.temp_label)+ "    and false label:" + str(self.root_label)+"   mask loc: " + str(self.res.mask_loc))
        
#        print(self.res.mask_loc)
        
        #################not good - It's just for 4 masks
        if self.res.mask_loc:      ##root's mask loc is none
