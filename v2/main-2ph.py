...
from util.model_tools_ret2 import train_and_test
...
if __name__ == '__main__':
    ...
    parser.add_argument('--load_all', action='store_true', default=False,
                        help='If True, optimizer and scheduler will be loaded as well as model.state unless just the weights will be loaded.')
    ...
    if args.first_dataset == 'cifar10':
        ...
        num_classes = 100
    if not args.first_model_load_path:
    ...
    #### TRAIN/TEST MODEL ####
    if not args.first_model_load_path:
        train_and_test(num_classes, csv_logger, args, trainloader, testloader, is_mango=False)
    ...
    if args.mango:
       #### USING MANGO AS A DATA TRANSFORM ####
        ...
        train_and_test(mng_num_classes, mng_csv_logger, args, is_mango = True)        
