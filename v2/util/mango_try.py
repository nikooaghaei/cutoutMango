...
from util.mask_color import far_edge, rand_pix
    ...
    def __call__(self, img, true_label = None):        
        ...
        while temp_mask_len > min_mask_len:
            ...
            for m, mask in enumerate(masks):
                ...
                if prob > max_prob: ######treshold comes here
                    max_prob = prob
                    branch = m
                    res = masked_img
            if branch == -1:
...
class Mng_Faredge(object):
    ...
    def __call__(self, imgs, true_labels = None):        
        ...
        while temp_mask_len > min_mask_len:
            ...
            for i in range(batch_size):
                masked_imgs[i] = torch.stack([
                far_edge(imgs[i], [y1[i], y2[i], x1[i], x2[i]]),
                far_edge(imgs[i], [y1[i], y2[i], x2[i], x3[i]]),
                far_edge(imgs[i], [y2[i], y3[i], x1[i], x2[i]]),
                far_edge(imgs[i], [y2[i], y3[i], x2[i], x3[i]])])
...
class MngCut_3x3(object):   ##not fixed
    """
